...
/* choose either bgcolor or bgimage */
 ...
 /* draw the grid lines on the radar every 30 degrees and write their values 180, 210, 240 etc.. */
  for (int i = 0; i <= 6; i++) {
    ...
    fill(255, 255, 255);
    noStroke();
    text(Integer.toString(0+(30*i)), radius + cos(radians(180+(30*i)))*SIDE_LENGTH/2, radius + sin(radians(180+(30*i)))*SIDE_LENGTH/2, 40, 50);
  }
...
else if (angle<0 & angle >80)
{
  ...
  fill(0, 300, 0);
  arial=createFont("arial.ttf",18);
  textFont(arial);
  textSize(18);
  text("Degrees: "+Integer.toString(Degrees), 50, 460);   text("degree", 200, 460);     
  text("Distance: "+Integer.toString(distance), 50, 480);  text("mm", 200, 480);
  text("0", 480, 500);
  text("100 mm", 480, 420);
  text("200 mm", 480, 320);
  text("300 mm", 480, 220);
